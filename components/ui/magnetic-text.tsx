...
export function MagneticText({ text, hoverText, className }: MagneticTextProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative inline-flex items-center justify-start cursor-none select-none",
        className,
      )}
    >
      {/* Base text */}
      ...
      {/* Magnetic circle overlay */}
      <div
        ref={circleRef}
        className="absolute top-0 left-0 pointer-events-none rounded-full bg-foreground overflow-hidden z-10"
        style={{
      ...
      >
        ...
        >
          <span className="text-background whitespace-nowrap font-bold tracking-tight">
            {hoverText ?? text}
          </span>
        </div>
      </div>
